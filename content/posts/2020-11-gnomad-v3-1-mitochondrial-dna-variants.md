...
* Heteroplasmic variants: Unlike nuclear variants, which are called as either heterozygous or homozygous (using GATK HaplotypeCaller), mtDNA variants are called at all heteroplasmy levels using a specialized mitochondria mode of the GATK MuTect2 variant caller (a variant caller that was originally designed to call somatic variants in tumor samples). MuTect2 calls are then stringently filtered as described below.
